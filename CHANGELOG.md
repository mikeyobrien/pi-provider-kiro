...
- Carry the SSO region through credential persistence, so a login outside `us-east-1` keeps refreshing against the region it was issued in. pi stores an OAuth credential as `access` / `refresh` / `expires` and hands back only those fields on the next session — which is why `clientId` and `clientSecret` are packed into the pipe-delimited refresh string in the first place. `region` was the one refresh-critical field left out of it, so `refreshKiroTokenDirect` read `credentials.region` as `undefined` and fell back to `us-east-1`: an IdC client registered in, say, `eu-west-1` was then presented to `oidc.us-east-1.amazonaws.com`, which answers `400 … invalid_grant` because the client does not exist there, and a `desktop` credential refreshed at the wrong `prod.{region}.auth.desktop.kiro.dev` host. The failure only appeared once the first access token aged out, and every other region consumer degraded the same way in the meantime — `refreshModels`, `modifyModels`, the post-login/post-refresh catalog fetch and `/settings` usage all resolved their API region from the same missing field and silently used `us-east-1`. The region is now appended after the auth-method marker, `src/refresh-token.ts` owns the one definition of the format, and every consumer resolves the region through `kiroCredentialRegion()`, which prefers the live field and falls back to the encoded one.
- Locate the auth-method marker in a refresh string by scanning for it rather than by taking the last segment. The previous positional read was correct only while the marker was last: it would have classified the region-carrying `refreshToken|desktop|eu-west-1` as `idc` and refreshed a social credential with an OIDC `refresh_token` grant. Strings written before this change have no region and still parse to exactly what they parsed to before, so an existing credential keeps refreshing at `us-east-1` until the next refresh, kiro-cli read, or Kiro IDE read re-encodes it with the region attached. The same parse now also backs the auth-family guard in `refreshKiroTokenInternal` (#142), which previously tested only for `desktop` and so treated an `external-idp` credential as `idc` — enough for it to be answered from Kiro IDE or kiro-cli IDC credentials belonging to a different account. `apikey` strings are unchanged: an API key is not issued against an SSO region.
- Address the model catalog and account usage at the region that actually served them, rather than at the SSO-derived region. `resolveKiroProfileArn` already probed both canonical management regions and remembered where the profile was found (#104, #131), but only `ListAvailableModels` consulted that memory: `GetUsageLimits` still went to the derived region, and `updateKiroModelsCache` stamped each cached model's `baseUrl` — and therefore the runtime host every request is sent to — with the region it had asked for, not the one that answered. A login whose SSO region maps to `eu-central-1` while the profile lives in `us-east-1` consequently cached a `us-east-1` catalog under an `eu-central-1` key pointing at `runtime.eu-central-1.kiro.dev`. `fetchKiroModelCatalog` now returns the serving region alongside the response, the cache records it as `catalogRegion`, and `modifyModels` reads it back. Cache entries written before this shipped have no `catalogRegion` and fall back to their own key, which is what they already meant.
- Serve a cached catalog from a canonical region when the requested key is absent. The cache is keyed by the SSO-derived region, so recovering the real region (above) can start asking for a key that no previous run ever wrote, which would drop every discovered model back to the static bootstrap list — including the model the user has selected, leaving `Model "kiro/…" not found` — until a network refresh happened to succeed. A catalog is regional to the profile rather than to the login, so a canonical-region entry answers correctly for a profile in that region.
